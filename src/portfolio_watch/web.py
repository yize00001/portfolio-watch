...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bottom { display: grid; grid-template-columns: 1fr 360px; gap: 24px; }
    table { width: 100%; border-collapse: collapse; background: #1e293b; border-radius: 12px; overflow: hidden; }
    th { background: #0f172a; padding: 12px 16px; text-align: left; font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: .05em; }
    td { padding: 14px 16px; border-top: 1px solid #0f172a; font-size: 0.875rem; }
    tr:hover td { background: #263348; }
    .pie-wrap { background: #1e293b; border-radius: 12px; padding: 20px; display: flex; flex-direction: column; }
    .pie-wrap h2 { font-size: 0.85rem; color: #64748b; margin-bottom: 16px; text-transform: uppercase; letter-spacing: .05em; }
    #updated { font-size: 0.75rem; color: #475569; text-align: right; margin-top: 8px; }
  </style>
</head>
<body>
  <header>
  ...
  <script>
    ...
    async function refresh() {
      ...
      tbody.innerHTML = d.rows.map(r => `
        <tr>
          <td><strong>${r.symbol}</strong><br><span style="color:#64748b;font-size:0.8rem">${r.name}</span></td>
          <td>${r.currency} ${fmt(r.price)}</td>
          <td class="${cls(r.change_percent)}">${sign(r.change_percent)}${fmt(r.change_percent)}%<br>
            <span style="font-size:0.8rem">${sign(r.daily_gain)}${fmt(r.daily_gain)}</span></td>
          <td>${fmt(r.market_value)}</td>
          <td class="${cls(r.unrealized_gain)}">
            ${sign(r.unrealized_gain)}${fmt(r.unrealized_gain)}<br>
            <span style="font-size:0.8rem">${sign(r.unrealized_gain_percent)}${fmt(r.unrealized_gain_percent)}%</span>
          </td>
        </tr>
      `).join('');
