...
from portfolio_watch.notifier import TelegramNotifier, format_alert
...
def _format_status(snapshots: list[PositionSnapshot]) -> str:
    lines = []
    for s in snapshots:
        lines.append(
...
class PortfolioBot:
    ...
    def run(self) -> None:
        logger.info("Bot started. Send /status to query portfolio.")
        while True:
            try:
